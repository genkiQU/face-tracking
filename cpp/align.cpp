...
// Depth Align program main function
int main(int argc, char* argv[]) {

	// file Names
	//std::string input_color = { argv[1] };
	std::string input_depth = { argv[1] };
	std::string depth_align_file = { argv[2] };
	...
	cv::Mat depthData = img2depth(depthImage);

	// align depth data to color camera
	cv::Mat depthAlignData = align(depthData, width, height, dK, dD, cT, cRM, cK);

	cv::imwrite(depth_align_file, depth2img(depthAlignData));
