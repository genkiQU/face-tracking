...
int main(int argc, char* argv[]) {
	...
	std::string output_color = { argv[2] };
	std::string output_depth = { argv[3] };
	...
	readingFile.open(input_ply, std::ios::in);
	std::string buf;
	char linedelimiter = ' ';

	std::vector<std::vector<double>> pointList;
	std::vector<std::vector<int>> colorList;
	while (std::getline(readingFile, buf)) {
		//std::cout << buf << std::endl;
		std::vector<std::string> points_and_colors = split(buf, linedelimiter);
		std::vector<double> points = { 0.0,0.0,0.0 };
		std::vector<int> colors = { 0,0,0 };

		for (int i = 0; i < 3; i++) {
			points[i] = (std::stod(points_and_colors[i]));
			colors[i] = int(std::stod(points_and_colors[i+3]));
		}
	...
	reprojectionPC_color(pointList, colorList, width, height, cK, output_color, output_depth);
